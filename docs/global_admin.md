...
**Global Admin** (identified as `SUPERADMIN` in the frontend user interface) is the highest account privilege tier in the HRMS platform. This account functions above the scope of individual organizations (tenants) and is responsible for managing the platform's overall infrastructure.

The Global Admin status is defined via the `is_global_admin` boolean property on the `User` model in the backend.
...
if is_internal:
    return self.get_response(request)  # Bypasses all tenant verification checks
```
...
| **Middleware Check** | **Bypass**. Totally immune to multi-tenant constraints. | **Enforced**. Confined by standard security middleware rules. |
...
Every time you invoke Django's default `createsuperuser` utility, the custom `UserManager` automatically appends the `is_global_admin=True` attribute to ensure full platform authorization:
```python
def create_superuser(self, email, password=None, **extra_fields):
    extra_fields.setdefault('is_staff', True)
    extra_fields.setdefault('is_superuser', True)
    extra_fields.setdefault('is_global_admin', True) # Automatically attached

# 🎯 HariKerja HRMS: Proyeksi Finansial Target Utama (Ultimate Target - 1 Juta Users)

Dokumen ini menyajikan rencana strategis, unit ekonomi, struktur biaya operasional, dan proyeksi keuntungan bersih untuk platform **HariKerja HRMS** dalam mencapai target utama (*Ultimate Target*): **1.000.000 Pengguna Aktif (Karyawan)**.
...
* **Basis Pengguna yang Dapat Dijangkau**: Terdapat sekitar 50–60 Juta pekerja formal di Indonesia. Target 1 juta pengguna aktif hanya mewakili **±2% pangsa pasar**.
* **Model Akuisisi B2B**: Target ini dicapai dengan mengakuisisi **20.000 Perusahaan/Tenant** dengan ukuran rata-rata **50 karyawan** per perusahaan.
...
$$\text{Gross Revenue} = 20.000 \text{ Tenant} \times \text{Rp } 450.000 = \mathbf{\text{Rp } 9.000.000.000 \text{ (Rp 9 Miliar / bulan)}}$$

---

## 📉 3. Struktur Biaya Operasional Bulanan (OpEx)

Untuk mengelola 1 juta karyawan aktif secara stabil di bawah infrastruktur cloud yang andal serta didukung oleh layanan pelanggan yang prima, dialokasikan anggaran bulanan sebagai berikut:
...
* **Pajak Penghasilan Badan (PPh Badan 22%)**:
  $$\text{PPh Badan} = 22\% \times \text{Rp } 6.570.000.000 = \mathbf{\text{Rp } 1.445.400.000 \text{ /bulan}}$$
* **Laba Bersih Setelah Pajak (Net Profit)**:
  $$\text{Net Profit} = \text{Rp } 6.570.000.000 - \text{Rp } 1.445.400.000 = \mathbf{\text{Rp } 5.124.600.000 \text{ /bulan}}$$
* **Margin Laba Bersih Akhir**: **56,94%**
...
* **Porsi Tenant Bulanan (70%)**: $14.000 \text{ tenant} \times \text{Rp } 450.000 = \text{Rp } 6.300.000.000$
* **Porsi Tenant Tahunan dengan Diskon 20% (30%)**: $6.000 \text{ tenant} \times (\text{Rp } 450.000 \times 0.8) = \text{Rp } 2.160.000.000$
* **Total Pendapatan Bulanan yang Disesuaikan**: **Rp 8.460.000.000 /bulan**
* **Laba Bersih Setelah Pajak Disesuaikan (Net Profit)**: 
  $$\text{EBITDA Baru} = \text{Rp } 8.460.000.000 - \text{Rp } 2.430.000.000 = \text{Rp } 6.030.000.000$$
  $$\text{Net Profit Baru} = \text{Rp } 6.030.000.000 \times 0.78 = \mathbf{\text{Rp } 4.703.400.000 \text{ /bulan}}$$
...
Untuk mencapai skala 1 juta pengguna aktif tanpa mengalami degradasi performa:

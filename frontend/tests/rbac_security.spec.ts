...
test.describe('RBAC Security & Permissions', () => {
  ...
  test('Employee should be restricted from Admin-only routes', async ({ page }) => {
    ...
    console.log('--- Testing Employee Restriction for /en/payroll ---');
    // Try to access Payroll Management (Admin only in seeded config)
    await page.goto('http://127.0.0.1:3000/en/payroll');
    await page.waitForLoadState('networkidle');
    
    // Should be restricted. We added a "Restricted Access" UI in PayrollPage.
    const restrictedTitle = page.getByRole('heading', { name: /Restricted Access/i });
    const payrollTitle = page.getByText(/Payroll Management/i);
    
    // Better diagnostic: check if we see "Restricted Access" heading and NOT the Admin-only page content
    await expect(restrictedTitle).toBeVisible({ timeout: 10000 });
    
    // The previous failure was because "Payroll Management" text appeared in the restriction message.
    // We check that the MAIN page heading (Admin-only) is NOT visible.
    const adminHeading = page.locator('main h1').getByText(/Payroll Management/i);
    await expect(adminHeading).not.toBeVisible();
    
    console.log('Employee successfully restricted from Payroll.');
  });

  test('Manager should have access to Performance but not Settings', async ({ page }) => {
    ...
    console.log('--- Testing Manager Restriction for /en/settings/branding ---');
    ...
    const brandingHeader = page.locator('main h1').filter({ hasText: /Branding/i });

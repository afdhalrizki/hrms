...
function Save-DockerComposeLogs {
    ...
    try {
        & docker compose --env-file (Join-Path $RootDir 'environments/.env.local') logs --no-color --tail 200 > $outFile 2>&1
...
function Start-BackendDevEnvironment {
    Write-Host "[Backend] Initializing dependencies and migrations via run_dev.ps1 --NoServer" -ForegroundColor Yellow
    Push-Location (Join-Path $RootDir "backend")
    # Use run_dev -NoServer to set up docker and migrations; does not start runserver.
    & pwsh -NoProfile -NoLogo -Command "./run_dev.ps1 -NoServer"
...
$suites = @(
    @{ Name = "Backend Stack"; Path = "backend\run_tests.ps1" },
    @{ Name = "Frontend Stack"; Path = "frontend\run_tests.ps1" },
    @{ Name = "Mobile Stack"; Path = "mobile\run_tests.ps1" }
)
